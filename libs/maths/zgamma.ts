...
export const zgamma = (function () {
  ...
  function zgamma(z: Complex): Complex {
    if (z.a < 0.5) {
      // pi / (sin(pi * z) * gamma(1 - z))
      return Complex.div(Math.PI, Complex.mult(Complex.sin(Complex.mult(Math.PI, z)), zgamma(Complex.sub(1, z))));
    } else {
      z = Complex.sub(z, 1);
